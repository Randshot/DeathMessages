...
public class DeathMessages extends JavaPlugin {
    ...
    private void initializeHooks() {
        ...
        if (!Bukkit.getPluginManager().isPluginEnabled("WorldGuard")
            ...
            getLogger().log(Level.SEVERE, "You enabled the WorldGuard hook in your settings.yml with WorldGuard not installed! Please disable!");
        ...
        if (Bukkit.getPluginManager().isPluginEnabled("PlugMan") && worldGuardExtension != null) {
            ...
            try {
                List<String> ignoredPlugins =
                        (List<String>) plugMan.getClass().getMethod("getIgnoredPlugins").invoke(plugMan);
        ...
        if (Bukkit.getPluginManager().getPlugin("WorldGuard") != null)
            try {
                ...
                if (worldGuardVersion < 7) {
                    getLogger().log(Level.SEVERE, "WorldGuard v" + worldGuardVersion + " is not supported yet. Please use WorldGuard v7 or higher!");
                    worldGuardExtension = null;
                } else {
